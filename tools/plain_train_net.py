        ...
        for epoch in range(start_epoch, max_epoch):
            ...
            for _ in range(iters_per_epoch):
                ...
                if iteration <= warmup_iters:
                    ...
                    print("#@#@#@", t_max_epoch)
                if epoch > t_max_epoch:
                    scheduler["lr_sched"].step(t_max_epoch)

            # Write metrics after each epoch
            for writer in writers:
                writer.write()
            print("###")
            if iteration > warmup_iters and (epoch + 1) > delay_epochs and epoch <= t_max_epoch:
                print("in")
